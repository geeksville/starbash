...
- **astro-color-stretch (ACS) stretch stage** (`doc/plans/astro-stretch.md`): David M. Jones' GPL `astro-color-stretch` 1.2 is ported as `src/starbash/recipes/astro_color_stretch.py` (per-channel zero-sky black point, root-power/asinh/log stretch, S-curve, ratio/HSV colour restore, optional CA/vignette/gradient correction, white balance, star reduction, RL deconvolution) plus the `post/astro-color-stretch.toml` recipe (46 parameters, `acs_` output prefix, VeraLux's pipeline slot). Seven deliberate deviations from upstream are documented in the module header (RGB order, RL luma weights, HSV scaling, dropped no-op call, mono raises, skimage keyword, and `ca_correct` defaulting off because the stage's input is the starless frame — §10 Q7 of the plan). `StretchParams` + `stretch_array()` are pure, so the GUI can reuse them for a preview later.
