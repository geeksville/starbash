...
- **Core architecture**:
  ...
  - `starbash.tool` — tool runners for Siril, GraXpert, Python (RestrictedPython), rc-astro (BlurXTerminator); handles safe template expansion
...
- Work items are `[[stage]]` tables across repos:
  - `tool`: one of `siril`, `graxpert`, `python`, `rc-astro` (see `starbash.tool.tools`). `rc-astro` runs the RC-Astro CLI (BlurXTerminator `bxt` and NoiseXTerminator `nxt`; nxt runs after blur-exterminator by default), always injects `--json`, and streams its JSON progress frames onto the event bus (`tool.progress`) via `tool_run_streaming()`; the CLI's `ProcessingView` and the GUI render them (tools never draw a Rich `Live` themselves — see `doc/plans/cli-live-display.md`). The `siril` `starnet` recipe (`starbash-recipes/common/starnet.toml`) runs after `sho` and emits `starless_*` + `starmask_*` outputs (declared as one output block with a two-element `name` list so the script can reference `output.full_paths[0]`/`[1]`). The `siril` `merge_stars` recipe (`starbash-recipes/post/merge_stars.toml`) runs after `veralux`, screen-blends the linear `starmask_<palette>.fits` (autostretched, scaled by the `merge_star_amount` parameter) back into the stretched `hms_starless_<palette>.fits`, and emits `merged_<palette>.fits`.
  ...
  - `[[stage.inputs.requires]]` filters (see `starbash.filtering._apply_filter`): `kind` is one of `metadata`, `camera`, `unprocessed`, `filename`, `min_count`. The `filename` filter keeps candidates whose basename matches a regex (`value`), with `mode = "include"` (default, keep matches) or `mode = "exclude"` (keep non-matches); e.g. VeraLux uses `filename`/`exclude`/`starmask` so it skips star masks while stretching the palette/starless outputs, and `merge_stars` uses `filename`/`include`/`hms_starless`. Any boolean-match `requires` node may add `invert = true` to keep the non-matching candidates (e.g. `palette/broadband.toml` selects sessions whose filter is neither HaOiii nor SiiOiii).
