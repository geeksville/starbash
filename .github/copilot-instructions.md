...
- Work items are `[[stage]]` tables across repos:
  ...
  - `[[stage.inputs.requires]]` filters (see `starbash.filtering._apply_filter`): `kind` is one of `metadata`, `camera`, `unprocessed`, `filename`, `min_count`. The `filename` filter keeps candidates whose basename matches a regex (`value`), with `mode = "include"` (default, keep matches) or `mode = "exclude"` (keep non-matches); e.g. VeraLux uses `filename`/`include`/`starless` so it only stretches starless (not starmask) files. Any boolean-match `requires` node may add `invert = true` to keep the non-matching candidates (e.g. `palette/broadband.toml` selects sessions whose filter is neither HaOiii nor SiiOiii).
