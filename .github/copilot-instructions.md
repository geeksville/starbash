...
- Work items are `[[stage]]` tables across repos:
  - `tool`: one of `siril`, `graxpert`, `python`, `rc-astro` (see `starbash.tool.tools`). `rc-astro` runs the RC-Astro CLI (BlurXTerminator `bxt` and NoiseXTerminator `nxt`; nxt runs after blur-exterminator by default), always injects `--json`, and streams JSON progress events to a live Rich progress bar via `tool_run_streaming()`. The `siril` `starnet` recipe (`starbash-recipes/common/starnet.toml`) runs after `sho` and emits `starless_*` + `starmask_*` outputs (declared as one output block with a two-element `name` list so the script can reference `output.full_paths[0]`/`[1]`).
