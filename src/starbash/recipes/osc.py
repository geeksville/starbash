...
def osc_process(has_ha_oiii: bool, has_sii_oiii: bool):
    ...
    logger.info(
        f"Running osc_process(has_ha_oiii={has_ha_oiii}, has_sii_oiii={has_sii_oiii})"
    )
    ...
    if has_sii_oiii:
        ...
        channel_num += 1
        make_stacked("SiiOiii", "Ha", f"results_{channel_num:05d}")

    if has_ha_oiii:
        # green output channel - from the HaOiii filter Ha is on the 656nm red channel
        channel_num += 1
        make_stacked("HaOiii", "Ha", f"results_{channel_num:05d}")
